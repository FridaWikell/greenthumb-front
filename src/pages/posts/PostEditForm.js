        ...
        <Col className="py-2 p-0 p-md-2" md={7} lg={8}>
          <Container className={`${appStyles.Content} ${styles.Container} d-flex flex-column justify-content-center`}>
            <Form.Group className="text-center">
              <figure><Image className={appStyles.Image} src={image} rounded /></figure>
            ...
            {errors.image?.map((message, idx) => <Alert key={idx} variant="warning">{message}</Alert>)}
          </Container>
        </Col>
          ...
          <Container className={appStyles.Content}>
            <div className="text-center">
              ...
              <Form.Group>
                ...
                <Form.Control as="select" name="hardinessZone" value={hardinessZone} onChange={handleChange}>
                  {Array.from({ length: 9 }, (_, index) => index).map(zone => (
                    <option key={zone} value={zone}>{`Zone ${zone}`}</option>
                  ))}
                </Form.Control>
                {errors.hardinessZone?.map((error, idx) => <Alert key={idx} variant="warning">{error}</Alert>)}
              </Form.Group>
